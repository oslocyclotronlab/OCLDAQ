...
#ifdef CAEN_NAI_ADC
static CAEN_ATDC_t vme_adc_1;
#endif
static CAEN_ATDC_t vme_adc_2, vme_adc_3, vme_adc_4, vme_tdc_1;
...
#ifdef CAEN_NAI_ADC
                CAEN_ATDC_clear(&vme_adc_1);
#endif
                CAEN_ATDC_clear(&vme_adc_2);
                CAEN_ATDC_clear(&vme_adc_3);
                CAEN_ATDC_clear(&vme_adc_4);
...
#ifdef CAEN_NAI_ADC
        CAEN_ATDC_data2buffer(&vme_adc_1);
#endif
        CAEN_ATDC_data2buffer(&vme_adc_2);
        CAEN_ATDC_data2buffer(&vme_adc_3);
        CAEN_ATDC_data2buffer(&vme_adc_4);
...
int main(int argc, char* argv[])
{
        ...
        {   unsigned short int tdc_timerange[1];
            if( !read_config_values("timerange_nai.txt", tdc_timerange, 1) )
                exit(-1);
            CAEN_ATDC_open(&vme_tdc_1, 0xAAA00000, /*box*/ 0x10, 1);
            CAEN_ATDC_config(&vme_tdc_1);
            CAEN_ATDC_config_timerange(&vme_tdc_1, tdc_timerange[0]);
        }
        ...
        {   unsigned short int adc_thr_nai[32];
            if( !read_config_values("thresholds_nai.txt", adc_thr_nai, 32) )
                exit(-1);
            CAEN_ATDC_open(&vme_adc_1, 0xAAC00000, /*box*/ 0x20, 0); // NaI
            CAEN_ATDC_config(&vme_adc_1, adc_thr_nai);
        }
#endif
        ...
        {   unsigned short int adc_thr_e[32];
            if( !read_config_values("thresholds_e.txt", adc_thr_e, 32) )
                exit(-1);
            CAEN_ATDC_open(&vme_adc_2, 0xAAB00000, /*box*/ 0x21, 0); // E
            CAEN_ATDC_config(&vme_adc_2, adc_thr_e);
        }

        {   unsigned short int adc_thr_de[64];
            if( !read_config_values("thresholds_de.txt", adc_thr_de, 64) )
                exit(-1);
            CAEN_ATDC_open(&vme_adc_3, 0xAAB10000, /*box*/ 0x22, 0); // DE
            CAEN_ATDC_config(&vme_adc_3, adc_thr_de);

            CAEN_ATDC_open(&vme_adc_4, 0xAAB20000, /*box*/ 0x23, 0); // DE
            CAEN_ATDC_config(&vme_adc_4, adc_thr_de+32);
        }
#ifdef MESYTEC_MADC32
        {   unsigned short int adc_thr_nai_mesytec[32];
            if( !read_config_values("thresholds_nai_mesytec.txt", adc_thr_nai_mesytec, 32) )
                exit(-1);
            MESYTEC_MADC32_open(&madc32, 0xAAD00000, /*box*/ 0x24); // mesytec MADC-32
            MESYTEC_MADC32_config(&madc32, adc_thr_nai_mesytec);
        }
#endif
        ...
        CAEN_ATDC_close(&vme_adc_4);
        CAEN_ATDC_close(&vme_adc_3);
        CAEN_ATDC_close(&vme_adc_2);
#ifdef CAEN_NAI_ADC
        CAEN_ATDC_close(&vme_adc_1);
#endif
