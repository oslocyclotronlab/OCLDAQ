...
Popup::Popup(const TGWindow * p, const TGWindow * main, string message)
{
  TGVerticalFrame *mn_vert = new TGVerticalFrame (this, 200, 300);

  TGGroupFrame *StateMsgFrame =
    new TGGroupFrame (mn_vert, "", kVerticalFrame);
  ...
  Message->Resize (150, 12);
  Message->SetEnabled (kFALSE);
  Message->SetFrameDrawn (kFALSE);
  Message->SetAlignment(kTextCenterX);

  StateMsgFrame->AddFrame(Message,new TGLayoutHints (kLHintsTop, 0, 0, 0, 0));
  mn_vert->AddFrame (StateMsgFrame,
		     new TGLayoutHints (kLHintsTop, 2, 5, 1,
					    5));
  fClient->GetColorByName ("red", color);
  Message->SetTextColor (color, true);
  Message->SetText (message.c_str());

  TGTextButton* OkButton = new TGTextButton (mn_vert, "&Ok", 
						 OK);
  OkButton->Associate (this);

  mn_vert->AddFrame (OkButton,
			new TGLayoutHints (kLHintsTop, 0, 20,
					   0, 0));
  AddFrame (mn_vert,
