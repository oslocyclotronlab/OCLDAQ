...
Histogram::Histogram(const TGWindow * p, const TGWindow * main, /*char **/string name,
		     int columns, int rows, int NumModules):Table(p, main,
								  columns,
								  rows,
								  name,
								  NumModules)
...
int Histogram::load_info(Long_t module)
{
  ...
  char text[20];
  char pEMIN[]="EMIN";
  char pBINFACTOR[]="BINFACTOR";

  for (int i = 0; i < 16; i++) {
    retval = Pixie16ReadSglChanPar(/*"EMIN"*/pEMIN, &ChanParData, modNumber, i);
    sprintf(text, "%d", (int) ChanParData);
    NumEntry[1][i]->SetText(text);

    retval =
      Pixie16ReadSglChanPar(/*"BINFACTOR"*/pBINFACTOR, &ChanParData, modNumber, i);
    sprintf(text, "%d", (int) ChanParData);
...
int Histogram::change_values(Long_t module)
{
  double cut;
  char pEMIN[]="EMIN";
  char pBINFACTOR[]="BINFACTOR";
  ...
  for (int i = 0; i < 16; i++) {
    ...
    Pixie16WriteSglChanPar(/*"EMIN"*/pEMIN, cut, modNumber, i);
    percent = NumEntry[2][i]->GetNumber();
    Pixie16WriteSglChanPar(/*"BINFACTOR"*/pBINFACTOR, percent, modNumber, i);
  }
