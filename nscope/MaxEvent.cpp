...
/////////////////column 1//////////////////////////////////////////
TGTextEntry *Dec = new TGTextEntry (column2, new TGTextBuffer (100), 10000,
									Dec->GetDefaultGC ()(),Dec->GetDefaultFontStruct (),
...
for (int i = 0; i < 3; i++)
  {
    retval = Pixie16ReadSglModPar (/*"MAX_EVENTS"*/pMAX_EVENTS, &ModParData,i);
...
for (int i = 0; i < 3; i++)
  {
   // d = maxEvent[i]->GetNumber ();
    retval=Pixie16WriteSglModPar (/*"MAX_EVENTS"*/pMAX_EVENTS, (unsigned long)d,i);
    std::cout<<"maxevent write(MAX_EVENTS): "<<retval  << "in mod "<<i<<std::endl;
    std::cout <<"********** " << "This function does not work, contact SNL for details " << " ********** " << std::endl; 
  }
