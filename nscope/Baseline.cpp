...
int
 Baseline::load_info(Long_t module) {
  double ChanParData = -1;

  int retval;
  char text[20];
  char pBLCUT[]="BLCUT";
  char pBASELINE_PERCENT[]="BASELINE_PERCENT";

  for (int i = 0; i < 16; i++) {
    retval = Pixie16ReadSglChanPar(/*"BLCUT"*/pBLCUT, &ChanParData, modNumber, i);
    sprintf(text, "%d", (int) ChanParData);
    NumEntry[1][i]->SetText(text);

    retval =
      Pixie16ReadSglChanPar(/*"BASELINE_PERCENT"*/pBASELINE_PERCENT, &ChanParData,
			      modNumber, i);
    sprintf(text, "%d", (int) ChanParData);
...
int
 Baseline::change_values(Long_t module) {
  double cut;
  char pBLCUT[]="BLCUT";
  char pBASELINE_PERCENT[]="BASELINE_PERCENT";
  ...
  for (int i = 0; i < 16; i++) {
    ...
    Pixie16WriteSglChanPar(/*"BLCUT"*/pBLCUT, cut, modNumber, i);
    percent = NumEntry[2][i]->GetNumber();
    Pixie16WriteSglChanPar(/*"BASELINE_PERCENT"*/pBASELINE_PERCENT, percent, modNumber, i);
  }
