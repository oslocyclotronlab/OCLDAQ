...
struct Event_t {
    ...
    sv::small_vector<uint32_t, 18> raw_data;
...
//    int size_raw;                       //! Size of the raw data in number of 32 bit words.
    ...
    Event_t(int64_t _timestamp, const uint32_t* begin, const uint32_t* end)
        : timestamp(_timestamp), raw_data(begin, end){}
};
...
private:
    ...
    // Temporary memory for various needs
